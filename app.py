...
def get_template_paths() -> tuple[Path | None, Path | None]:
    candidate_roots = [Path.cwd(), Path.cwd().parent, Path(__file__).resolve().parents[1]]
    guideline_path = None
    category_path = None
    ...
    for root in candidate_roots:
        ...
        if possible_guidelines.exists():
            guideline_path = possible_guidelines
        if possible_categories.exists():
            category_path = possible_categories

    return guideline_path, category_path
